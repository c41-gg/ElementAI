import pygame
from constants import BLACK, WIDTH, HEIGHT, label_player_image, BEIGE
...
class Player(object):
    ...
    def win(self, astra):
        self.stack += int(astra)
        self.win_astra += astra

    def drop(self, astra):
        self.stack -= int(astra)
        self.input_stack += astra
        self.decision = True
        self.bet_auction += astra

    def blind(self, astra):
        self.stack -= astra
        self.input_stack += astra
        self.bet_auction += astra
    ...
    def draw_pot(win):
        ...
        font = pygame.font.SysFont('Roboto', 20)
        text = font.render('{}'.format(str(input_stack - bets)), True, BEIGE)
