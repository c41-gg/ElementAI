...
def recap_round(list_winner, common_cards=None):
    ...
    from constants import WIN, BACKGROUND, WIDTH, HEIGHT, BEIGE
...
def give_card(type_card, cards):
    ...
    import pygame
    from constants import cards_object
...
def arrange_room(common_cards=None):
    ...
    from constants import WIN, BACKGROUND
    from player import Player
    player_list_chair = Player.player_list_chair
...
def player_decision(buttons, dict_options, min_raise, max_raise, common_cards=None):
    ...
    from constants import WIN, BEIGE
    from button import x_buttons, y_button, width_button
...
def split_pot():
    ...
    # Calculates how many players will be given back the astra they have put into the main pot
    for i in range(n):
    ...
    # Calculates how many each player wins the astra
    for i in range(n):
...
def create_deck():
    suits = ['Fire', 'Earth', 'Water', 'Air']
    types = ['Knight', 'Mage', 'Ranger']
    deck = []

    for suit in suits:
        for type_ in types:
            for _ in range(3):  # Three copies of each type
                deck.append(f'{type_} of {suit}')
